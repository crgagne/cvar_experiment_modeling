...
def add_triangles(ax,maze,cm,add_labels=False,fs=5,cm_empty=10,ec=(0.4,0.4,0.4,0.1),ls='-',term_states_for_plotting=[],order='C'):
    #print(ls)
    triangles = {}
    labels = {}
    ...
    for x in range(maze.shape[1]): # these are reversed
        for y in range(maze.shape[0]):
            ...
            if not idcs2state([y,x],maze,order=order) in term_states_for_plotting:
                ...
                s = 0.75 # scaling
                triangles[str(x)+'_'+str(y)+'_up']=plt.Polygon([[x-0.2*s,y-0.25], [x+0.2*s,y-0.25], [x+0,y-0.45]],fc=cm[cm_empty],ec=ec,alpha=1,linestyle=ls)
                triangles[str(x)+'_'+str(y)+'_down']=plt.Polygon([[x-0.2*s,y+0.25], [x+0.2*s,y+0.25], [x+0,y+0.45]],fc=cm[cm_empty],ec=ec,alpha=1,linestyle=ls)
                triangles[str(x)+'_'+str(y)+'_left']=plt.Polygon([[x-0.25,y-0.2*s], [x-0.25,y+0.2*s], [x-0.45,y+0]],fc=cm[cm_empty],ec=ec,alpha=1,linestyle=ls)
                triangles[str(x)+'_'+str(y)+'_right']=plt.Polygon([[x+0.25,y-0.2*s], [x+0.25,y+0.2*s], [x+0.45,y+0]],fc=cm[cm_empty],ec=ec,alpha=1,linestyle=ls)
            ...
            if s not in term_states_for_plotting:
                ...
                try:
                    ...
                    if np.abs(new_q)<min_plot:
                        #import pdb; pdb.set_trace()
                        trianglelist[0]['_'.join(str(x) for x in si[::-1])+'_'+action2str(a,num_actions=num_actions)].set_fc([1.,1.,1.])
                        trianglelist[0]['_'.join(str(x) for x in si[::-1])+'_'+action2str(a,num_actions=num_actions)].set_ec(None)
                        trianglelist[0]['_'.join(str(x) for x in si[::-1])+'_'+action2str(a,num_actions=num_actions)].set_visible(False)
                    else:
...
def plot_q_or_pi(Q,V,title,ax,maze,q_or_pi='q',Qrange=None,roundoff=3,annot_value=False,
    ...
    plot_value=True,term_states_for_plotting=[],pi_color="blue",tri_type='all',colorbar=False,order='C',min_plot=0.01):
    ...
    if inc_triangles:
        if tri_type=='all':
            tri,lab = add_triangles(ax,maze,cm_Q,add_labels=tri_add_labels,cm_empty=cm_empty,ec=tri_ec,ls=tri_ls,fs=tri_fs,term_states_for_plotting=term_states_for_plotting,order=order)
...
def plot_sa(Q,V,task,alpha_set,alpha_idx,model_name,q_or_pi='q',
            ...
            pi_color='blue',min_plot=0.01,title_fs=20,start_fs=16,reward_fs=16,value_fs=14,show_alpha0=False):
    ...
    ab = AnnotationBbox(imagebox, (2, 2.9),pad=0,frameon=False)
    ax.add_artist(ab)

    # add some extra stuff
    ax.text(0,3,'start',fontsize=start_fs,color='k',
            bbox=dict(edgecolor='white',facecolor='white', alpha=1,pad=2),ha='center', va='center')
    ax.text(6,3,'goal',fontsize=start_fs,color='k',
            bbox=dict(edgecolor='white',facecolor='white', alpha=1,pad=2),ha='center', va='center')
