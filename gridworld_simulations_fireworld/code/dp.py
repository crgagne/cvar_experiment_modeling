...
def CVaR_DP(task,
    ...
    for t in reversed(range(0, T)):
        ...
        # loop over state space a
        if parallel:
            with Pool(multiprocessing.cpu_count()) as p:
                map_results = p.starmap(Update_Q_Values, [(s, t, V_CVaR, Nalpha, n_actions, n_rewards,
                                                           n_states, T, alpha_set, task, cvar_type, gamma, interp_type,
                                                           same_answer_ns) for s in states_to_iterate])
        else:
            map_results = []
            ...
            for alpha_i, alpha in enumerate(alpha_set):
                ...
                if policy_to_evaluate is None:
                    if cvar_type == 'fCVaR':
                        ...
                        Q_best_alpha0 = np.nanmax(Q_CVaR[s, actions_allowed, alpha0_i, t])
                        best_actions = np.where(np.squeeze(Q_CVaR[s, :, alpha0_i, t]) == Q_best_alpha0)[0]
                        #print("state: {} , alpha0: {} , alpha in loop: {}, best actions: {}".format(s, alpha0, alpha, best_actions))
                        ...
                        Q_CVaR[s, :, alpha_i, t] = np.round(Q_CVaR[s, :, alpha_i, t],
                                                            Q_roundoff)  # round Q-values so that 'numerical ties' are obvious; not necessary but cleaner for looking at policy
...
def interpolate_V(V, state_next, alpha_next, alpha_set, t, debug=False):
    ...
    if alpha_next > 1:
        ...
        # print('no inerpolation, value: {} \n'.format(state_values[state_next, alpha_next_i_nearest, t]))
        ...
        # Slope
        slope = (V[state_next, alpha_next_i_upper, t] - V[state_next, alpha_next_i_lower, t]) / (
                    alpha_next_upper - alpha_next_lower)
        # print('Interolation at step: {} in state {}\n'.format(t, state_next))
        # print('value_alpha_lower{}\n'.format(state_values[state_next, alpha_next_i_lower, t]))
        # print('value_alpha_upper{}\n'.format(state_values[state_next, alpha_next_i_upper, t]))
        ...
        # print('Interpolation_index: {}\n'.format(alpha_next_i_lower))
        # print('alpha_next: {}, alpha_next_lower: {}\n'.format(alpha_next , alpha_next_lower))
        # print('Interpolated state_values: {}\n'.format(V_interp))
...
def distorted_value_objective_fun(dist_weights,
    ...
    for (next_state, reward), prob, dweight in zip(next_state_reward_pairs,
        ...
        if cvar_type == 'pCVaR' or cvar_type == 'fCVaR':
            #print('Value for all alphas in state {} at step {}\n'.format(next_state, t+1) + '  ' + str(state_values[next_state, :,t + 1]))
...
def Q_backup(next_states,
    ...
    prob_next_state_reward_pairs = [probs[0] * probs[1] for probs in
                                    itertools.product(p_next_states, p_rewards)]  # indedendent so multiply
    ...
    if alpha == 0:
        ...
        def sum_to_1_constraint(dist_weights):
            zero = np.dot(dist_weights,
                          np.array(prob_next_state_reward_pairs)) - 1  # p_trans is found in one python env up
            return (zero)
...
def Q_backup_horizon(rewards,
    ...
    if alpha == 0.0:
        ...
        dist_weights[minR_idcs] = 1 / (p_rewards[minR_idcs] * len(
            minR_idcs))  # take the weight as 1/prob, but if there are more than 1 additionally divide by total)
